...
* Нет доступа из клиентского Openshift в разработческое окружение.
* По ряду идиологических причин Не было возможности переиспользовать существующие Docker образы для разработки.
...
* Использоввать start hook.
* Сделать отдельный контейнер, который содержит конфигурацию и применит ее к приложению. Грубо аналог миграция для БД.

Мы выбрали последний подход, т.к. он позволяет делать конфигурацию воспроизводимой и самодостаточной. Только зачему-то изначально сделали этот контейнер в отдельном replication controller с фактором 1.
...
У нас получилась схема как выглядит приложение развернутое. Теперь настало время обсудить инструментарий в природе существует много уже готового, я рассмотрю некоторые из списка ниже и сделаю субьективные выводы.
...
* Если у вас есть зависимости межд сервисами и их очередностью старта, то будет сложно.
...
**Плюсы:**
* Вы не беспокоетесь об очередности создания элементов инфраструктуры.
* Можно переиспользовать код инфраструктуры как модули.
* Можно добавить логику инциализации приложения.
...
* Выгялдит покинутым и не поддерживаемым. Был заменен [Ansible bender](https://blog.tomecek.net/post/road-to-ansible-bender-0-2-0/).
...
**Минусы:**
* Вам нужны привелегии уровеня администратора.
